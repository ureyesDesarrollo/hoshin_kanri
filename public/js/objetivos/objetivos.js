let filtroTituloObj = "";
let filtroResponsableObj = "";
let searchTimerObj = null;

function loadObjetivosStats(periodoId) {
  $.get(
    "/hoshin_kanri/app/objetivos/stats.php",
    {
...
function loadObjetivos(page = 1, periodoId) {
  ...
  $.get(
    "/hoshin_kanri/app/objetivos/lista.php",
    {
      periodo_id: periodoId,
      page: page,
      q: filtroTituloObj,
      responsable: filtroResponsableObj,
    },
    function (resp) {
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                ${o.responsable_email ? `<div class="small text-muted">${o.responsable_email}</div>` : ""}
                            </div>
                        </div>
                    </td>
...
$(document).ready(function () {
  ...
  // Buscador por título
  $("#buscarTituloObjetivo").on("input", function () {
    filtroTituloObj = $(this).val().trim();
    clearTimeout(searchTimerObj);
    searchTimerObj = setTimeout(() => loadObjetivos(1, periodoId), 250);
  });

  // Buscador por responsable
  $("#buscarResponsableObjetivo").on("input", function () {
    filtroResponsableObj = $(this).val().trim();
    clearTimeout(searchTimerObj);
    searchTimerObj = setTimeout(() => loadObjetivos(1, periodoId), 250);
  });

  // Limpiar
  $("#btnLimpiarBusquedaObjetivos").on("click", function () {
    filtroTituloObj = "";
    filtroResponsableObj = "";
    $("#buscarTituloObjetivo").val("");
    $("#buscarResponsableObjetivo").val("");
    loadObjetivos(1, periodoId);
  });
  ...
  $("#paginacionObjetivos").on("click", "a.page-link", function (e) {
    ...
    if (
      !page ||
      $(this).closest(".page-item").hasClass("disabled") ||
      $(this).closest(".page-item").hasClass("active")
    )
      return;
