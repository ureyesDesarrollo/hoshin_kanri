function loadTareasStats(periodoId) {
    $.get('/hoshin_kanri/app/tareas/stats.php', {
...
function loadMilestonesModal(milestone_id = 0) {
    ...
    if (!estrategiaId) {
        $('#tareaMilestoneId').append('<option value="">Seleccione una estrategia</option>');
        return;
    }

    $.get('/hoshin_kanri/app/milestone/listar.php', { estrategia_id: estrategiaId }, function (resp) {
        if (!resp.success) {
            Swal.fire('Error', resp.message || 'Error cargando milestones', 'error');
            return;
        }

        $('#tareaMilestoneId').append('<option value="">Seleccionar milestone</option>');
...
function loadEstrategiaModal(selectedId = 0) {
    ...
    $('#estrategiaTarea').html('<div class="text-muted small p-3">Cargando estrategias...</div>');

    $.get('/hoshin_kanri/app/estrategias/listar_estrategias.php', function (resp) {
        if (!resp.success) {
            $('#estrategiaTarea').html('<div class="text-danger p-3">Error cargando estrategias</div>');
            return;
        }
...
function loadTareas(page = 1) {
    ...
    $.get('/hoshin_kanri/app/tareas/lista.php', {
        page: page
    }, function (resp) {
...
function onEstrategiaChange(nuevaId) {
    ...
    // reset milestones
    $('#tareaMilestoneId').empty().append('<option value="">Seleccionar milestone</option>');
...
$(document).on('click', '#estrategiaTarea .obj-card', function () {
    ...
    $('#estrategiaTarea .obj-card').removeClass('active')
...
$(document).ready(function () {
    ...
    $('#tablaTareas').on('click', '.btnEditarTarea', function () {
        ...
        $.get('/hoshin_kanri/app/tareas/listar_por_id.php', { id }, function (resp) {
            if (!resp.success) return;
    ...
    $('#btnGuardarTarea').on('click', function () {
        ...
        if (tarea_id) {
            $.ajax({
                ...
                data: {
                    tarea_id, titulo, responsable_id, descripcion, milestone_id, fecha_inicio, fecha_fin
                },
            ...
            $.ajax({
                ...
                data: {
                    titulo, responsable_id, descripcion, milestone_id, fecha_inicio, fecha_fin
                },
                success: function (resp) {
                    if (resp.success) {
                        swal.fire({
