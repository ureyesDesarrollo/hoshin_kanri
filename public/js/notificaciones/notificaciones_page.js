...
function renderNotifRow(n) {
    ...
    const ctxParts = [];
    if (n.tarea_titulo) ctxParts.push(`<i class="fas fa-tasks me-1"></i>${escapeHtml(n.tarea_titulo)}`);
    if (n.milestone_titulo) ctxParts.push(`<span class="ms-2"><i class="fas fa-flag-checkered me-1"></i>${escapeHtml(n.milestone_titulo)}</span>`);
    if (n.estrategia_titulo) ctxParts.push(`<span class="ms-2"><i class="fas fa-chess-knight me-1"></i>${escapeHtml(n.estrategia_titulo)}</span>`);

    const ctx = ctxParts.length ? ctxParts.join('') : '';
    const resp = n.tarea_responsable ? `<div class="notif-body"><i class="fas fa-user me-1"></i>${escapeHtml(n.tarea_responsable)}</div>` : '';
...
function loadNotificaciones(page = 1) {
    ...
    $.get('/hoshin_kanri/app/notificaciones/lista.php', { page, limit: 15, f, q }, function (resp) {
        $('#notifEstadoCargando').addClass('d-none');

        if (!resp || !resp.success) {
            $('#notifEstadoVacio').removeClass('d-none').text('Error al cargar notificaciones');
            return;
        }
...
function marcarLeida(notificacionId, cb) {
    $.post('/hoshin_kanri/app/notificaciones/marcar_leida.php', { notificacion_id: notificacionId }, function () {
...
$(document).ready(function () {
    ...
    $('#btnNotifMarcarTodas').on('click', function () {
        $.post('/hoshin_kanri/app/notificaciones/marcar_todas.php', {}, function (resp) {
            if (resp && resp.success) {
                Swal.fire({ icon: 'success', title: 'Listo', text: 'Se marcaron como leídas.' });
                loadNotificaciones(1);
            }
        }, 'json');
    });
